...
#define TMRCFG_REG(N)                                                          \
    TMR_GEN_CALC(0x100, N) /* Timer N Configuration and Capability Register */
#define TMRCMP_REG(N)                                                          \
    TMR_GEN_CALC(0x108, N) /* Timer N Comparator Value Register */
#define TMRFSB_REG(N)                                                          \
    TMR_GEN_CALC(0x110, N) /* Timer N FSB Interrupt Route Register */

volatile bool hpetSleepFlag = false;

static inline uint8_t findIrq(uint32_t irqs)
{
    uint8_t bitpos = 0;
...
struct hpet_timer* hpetCreateTimer()
{
    ...
    pgAddGlobalPage((uint64_t)timer->BaseAddress, (uint64_t)timer->BaseAddress,
    ...
    uint64_t cap = timer->BaseAddress[GENCAP_REG];
    {
        ...
        if ((cap & (1 << 15))) {
            ...
            uint32_t irqs =
                timer->BaseAddress[TMRCFG_REG(timer->TimerNumber)] >> 32;
    ...
    timer->Base.MillisecondScale = (unsigned long)1e+12 / (unsigned long)timer->TimerPeriod;
    timer->BaseAddress[TMRCFG_REG(timer->TimerNumber)] =
        (timer->ApicLine << 9) | (1 << 2);
    return timer;
}
...
void setReloadValue(struct timer* cls, uint64_t reload)
{
    ...
    /* disable the timer */
    hpetSleepFlag = false;
    this->BaseAddress[TMRCMP_REG(this->TimerNumber)] =
        this->BaseAddress[GENMCV_REG] + reload;
...
uint64_t getMillisecondUnit(struct timer* cls) { return 1e+12; }
